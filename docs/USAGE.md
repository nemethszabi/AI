...
| **Your PR got review comments** — fix them | `/pr:fix <PR>` — fixes on the source branch, builds, tests; commit, push (only after a commit, SHA shown), thread replies and thread statuses are four separate approvals. `--from-review` fixes your own `/pr:review` findings before anyone has to comment. Agent-reviewed; the first real run is a proving run | Global |
