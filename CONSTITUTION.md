...
1. `d:\_AI_GIT\` (and its live copies — `~\.claude\` for Claude Code; `~\.copilot\` for GitHub Copilot CLI,
   once its rollout is approved) is reserved exclusively for generic, reusable dev/work tooling — agents,
   skills, commands, and doctrine meant for software-engineering and professional work.
...
2. Personal-life behavior belongs under `d:\WORK\AI\.claude\` instead — scoped there the same way a
   project's own `.claude\` scopes its behavior to that project, not copied into any shared tool config's
   global layer (`~\.claude\`, `~\.copilot\`). Applying the existing generic-vs-project-specific placement
   test (does the artifact
...
here take effect globally only once copied to each tool's live config root
(`~\.claude\CONSTITUTION.md`, `~\.copilot\CONSTITUTION.md`) — see `claude\README.md`'s and
`copilot\README.md`'s own Rollout sections.
