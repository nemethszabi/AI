...
it exists) against `ai/sa/<slug>/requirements.md` via `req-reviewer`, writing `ai/sa/<slug>/review.md`. Run
this after `/sa:design` and before `/sa:design-detail` — catching a coverage gap or a shaky alternative in
the HLD is cheaper than catching it after the LLD is built on top of it. Re-run after `/sa:design-detail`
too, if you want the LLD checked against the HLD as well.
...
<step name="relay">
Return the agent's summary (finding count by severity, REQ-coverage headline), which model actually ran the
review (so a `--model=opus` request is visibly confirmed, not just assumed), and the file path written.
