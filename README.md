...
| `commands\sa\ingest.md`, `clarify.md`, `design.md`, `review.md`, `design-detail.md`, `estimate.md`, `doc.md`, `help.md` | `/sa:*` — lightweight, generic REQ/CR pipeline (ingest → clarify → design(HLD) → review → design-detail(LLD) → estimate → doc), no formal gates anywhere. Dispatches to `req-ingestor`/`req-analyst`/`req-architect`/`req-reviewer`/`req-detailer`/`req-estimator`; `doc` consolidates directly (and now also surfaces the HLD's Quality Attributes/Security & Compliance highlights, 2026-08-12). `design`/`design-detail` also dispatch `mermaid-diagram-maker` afterward to render whatever diagrams the HLD/LLD named, as paired `.mmd`+`.png` files under `ai/sa/<slug>/diagrams/`. Every step optional/skippable — `ingest` only matters when starting from files, the HLD/LLD split only matters for topics big enough to need it. Artifacts slugged per-topic under `ai/sa/<slug>/` so repeated use doesn't collide. `ingest`/`review`/`design-detail` added 2026-08-11 to cover Excel/Word document analysis and an HLD→LLD progression with a review gate between them. `design` takes `--model=<name>` (syntax aligned across the namespace 2026-08-12) to escalate the `req-architect` dispatch for a genuinely novel/high-stakes design, and `--apply-review[=<severity>]` to revise an existing HLD against `review.md` findings instead of a fresh design. |
